...
impl WgslGen {
    ...
    fn emit_header(&mut self) {
        ...
        self.line("// https://github.com/your/game-engine");
        self.blank();
    }
    ...
    fn emit_builtin_functions(&mut self) {
        ...
        // FBM with persistence + lacunarity
    ...
    fn emit_flat_stage(&mut self, stage: &FnCall, _prev: &ShaderState, index: usize) -> Result<()> {
        ...
        match stage.name.as_str() {
            ...
            "bloom" => {
                ...
                self.line(&format!("let pp_lum = dot(color_result.rgb, vec3f(0.299, 0.587, 0.114));"));
    ...
    fn emit_raymarch_fragment(
        ...
        let chain = layer.fn_chain.as_ref().unwrap();
    ...
    fn classify_stage(&self, name: &str) -> Result<ShaderState> {
        match name {
            ...
            "shade" | "emissive" | "colormap" | "spectrum" => Ok(ShaderState::Color),
