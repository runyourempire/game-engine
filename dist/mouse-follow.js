// mouse-follow.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component. Import and use:
//   <script type="module" src="./mouse-follow.js"></script>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    let trail_x = u.p_trail_x;
    let trail_y = u.p_trail_y;
    let mx = u.p_mx;
    let my = u.p_my;
    ...
    // ── Layer 1: trail ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: translate(...)
        p = p - vec2f(trail_x, trail_y);

        // stage 1: circle(...)
        var sdf_result = sdf_circle(p, 0.12);
        ...
        color_result = vec4f(color_result.rgb + max(pp_lum - 0.3, 0.0) * 1.0, 1.0);
...
class MouseFollow extends HTMLElement {
  ...
  _frame() {
    ...
    // Update param values
    this._paramValues[0] = (this._arcBaseOverrides?.[0] ?? 0) + ((mouseX * 1.8) - 0.9);
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 0) + ((mouseY * 1.8) - 0.9);
    this._paramValues[2] = (this._arcBaseOverrides?.[2] ?? 0) + ((mouseX * 2.0) - 1.0);
    this._paramValues[3] = (this._arcBaseOverrides?.[3] ?? 0) + ((mouseY * 2.0) - 1.0);
...
customElements.define('mouse-follow', MouseFollow);
export { MouseFollow };
