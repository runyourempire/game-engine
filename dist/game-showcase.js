// game-showcase.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component with WebGL2 fallback.
//   <script type="module" src="./game-showcase.js"></script>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    let bass = u.p_bass;
    let mid = u.p_mid;
    let treble = u.p_treble;

    var p = vec2f(uv.x * aspect, uv.y);
    // stage 0: spectrum(...)
    let sp_bass = bass;
    let sp_mid = mid;
    let sp_treble = treble;

    // Concentric rings — each frequency band at a different radius
    let d_bass = abs(length(p) - 0.15) - 0.02;
    let d_mid = abs(length(p) - 0.35) - 0.015;
    let d_treble = abs(length(p) - 0.55) - 0.01;

    // Core glow (always-on subtle center)
    let sp_core = exp(-length(p) * 6.0) * 0.12;

    // Band-reactive glows — sharpen + intensify with signal
    let g_bass = exp(-max(d_bass, 0.0) * (4.0 + sp_bass * 20.0)) * sp_bass;
    let g_mid = exp(-max(d_mid, 0.0) * (6.0 + sp_mid * 25.0)) * sp_mid;
    let g_treble = exp(-max(d_treble, 0.0) * (8.0 + sp_treble * 30.0)) * sp_treble;

    // Frequency-specific colors
    let c_bass = vec3f(1.0, 0.3, 0.05);
    let c_mid = vec3f(0.05, 1.0, 0.7);
    let c_treble = vec3f(0.6, 0.15, 1.0);

    var color_result = vec4f(
        sp_core * vec3f(0.5, 0.4, 0.3) +
        g_bass * c_bass +
        g_mid * c_mid +
        g_treble * c_treble,
        1.0
    ...
    color_result = vec4f(color_result.rgb + max(pp_lum - 0.4, 0.0) * 2.0, 1.0);

    // stage 2: chromatic(...)
    let ca_d = length(uv) * length(uv) * 0.6;
    color_result = vec4f(color_result.r * (1.0 + ca_d * 0.5), color_result.g, color_result.b * (1.0 - ca_d * 0.3), 1.0);

    // stage 3: vignette(...)
    color_result = vec4f(color_result.rgb * (1.0 - length(uv) * 0.35), 1.0);

    // stage 4: grain(...)
    let gr_n = fract(sin(dot(input.uv * (time * 97.0 + 1.0), vec2f(12.9898, 78.233))) * 43758.5453) * 2.0 - 1.0;
    color_result = vec4f(color_result.rgb + gr_n * 0.025, 1.0);
...
uniform float u_p_bass;
uniform float u_p_mid;
uniform float u_p_treble;
...
void main() {
    ...
    float bass = u_p_bass;
    float mid = u_p_mid;
    float treble = u_p_treble;

    vec2 p = vec2(uv.x * aspect, uv.y);
    // stage 0: spectrum(...)
    float sp_bass = bass;
    float sp_mid = mid;
    float sp_treble = treble;

    // Concentric rings — each frequency band at a different radius
    float d_bass = abs(length(p) - 0.15) - 0.02;
    float d_mid = abs(length(p) - 0.35) - 0.015;
    float d_treble = abs(length(p) - 0.55) - 0.01;

    // Core glow (always-on subtle center)
    float sp_core = exp(-length(p) * 6.0) * 0.12;

    // Band-reactive glows — sharpen + intensify with signal
    float g_bass = exp(-max(d_bass, 0.0) * (4.0 + sp_bass * 20.0)) * sp_bass;
    float g_mid = exp(-max(d_mid, 0.0) * (6.0 + sp_mid * 25.0)) * sp_mid;
    float g_treble = exp(-max(d_treble, 0.0) * (8.0 + sp_treble * 30.0)) * sp_treble;

    // Frequency-specific colors
    vec3 c_bass = vec3(1.0, 0.3, 0.05);
    vec3 c_mid = vec3(0.05, 1.0, 0.7);
    vec3 c_treble = vec3(0.6, 0.15, 1.0);

    vec4 color_result = vec4(
        sp_core * vec3(0.5, 0.4, 0.3) +
        g_bass * c_bass +
        g_mid * c_mid +
        g_treble * c_treble,
        1.0
    ...
    color_result = vec4(color_result.rgb + max(pp_lum - 0.4, 0.0) * 2.0, 1.0);

    // stage 2: chromatic(...)
    float ca_d = length(uv) * length(uv) * 0.6;
    color_result = vec4(color_result.r * (1.0 + ca_d * 0.5), color_result.g, color_result.b * (1.0 - ca_d * 0.3), 1.0);

    // stage 3: vignette(...)
    color_result = vec4(color_result.rgb * (1.0 - length(uv) * 0.35), 1.0);

    // stage 4: grain(...)
    float gr_n = fract(sin(dot(v_uv * (time * 97.0 + 1.0), vec2(12.9898, 78.233))) * 43758.5453) * 2.0 - 1.0;
    color_result = vec4(color_result.rgb + gr_n * 0.025, 1.0);

    fragColor = color_result;
}
`;

class GameShowcase extends HTMLElement {
  ...
  _frame() {
    ...
    // Update param values
    this._paramValues[0] = (this._arcBaseOverrides?.[0] ?? 0) + (audioBass * 1.5);
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 0) + (audioMid * 1.5);
    this._paramValues[2] = (this._arcBaseOverrides?.[2] ?? 0) + (audioTreble * 1.5);
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_bass');
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_mid');
    this._glParamUniforms[2] = gl.getUniformLocation(prog, 'u_p_treble');
