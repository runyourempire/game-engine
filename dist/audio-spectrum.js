// audio-spectrum.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component. Import and use:
//   <script type="module" src="./audio-spectrum.js"></script>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    let bass_g = u.p_bass_g;
    let mid_g = u.p_mid_g;
    let treble_g = u.p_treble_g;
    let energy_g = u.p_energy_g;
    ...
    // ── Layer 0: bg ──
    {
        ...
        // stage 0: gradient(...)
        var color_result = vec4f(mix(vec3f(0.0, 0.02, 0.15), vec3f(0.0), length(uv)), 1.0);
    ...
    // ── Layer 1: bass_ring ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.15) - 0.04;

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, bass_g);
    ...
    // ── Layer 2: mid_ring ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.25) - 0.03;

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, mid_g);
    ...
    // ── Layer 3: treble_ring ──
    {
        ...
        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, treble_g);
    ...
    // ── Layer 4: core ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: circle(...)
        var sdf_result = sdf_circle(p, 0.06);

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, energy_g);
...
class AudioSpectrum extends HTMLElement {
  ...
  _frame() {
    ...
    this._paramValues[3] = (this._arcBaseOverrides?.[3] ?? 2) + (audioEnergy * 8.0);
...
customElements.define('audio-spectrum', AudioSpectrum);
