...
uniform float u_p_ring_glow;
uniform float u_p_dot_glow;
...
void fs_main(input: VertexOutput){
    ...
    float ring_glow = u_p_ring_glow;
    float dot_glow = u_p_dot_glow;
    ...
    // ── Layer 0: bg_ring ──
    {
        ...
        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, 0.5);
    ...
    // ── Layer 1: active_ring ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - ring_r) - 0.03;

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, ring_glow);
    ...
    // ── Layer 2: center ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: circle(...)
        float sdf_result = sdf_circle(p, 0.05);

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, dot_glow);

        // stage 2: tint(...)
        vec4 color_result = vec4(vec3(glow_result) * vec3(0.831, 0.686, 0.216), 1.0);
...
class LoadingStages extends HTMLElement {
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_ring_glow');
    this._glParamUniforms[2] = gl.getUniformLocation(prog, 'u_p_dot_glow');
