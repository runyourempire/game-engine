...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    // stage 4: vignette(...)
    color_result = vec4f(color_result.rgb * (1.0 - length(uv) * 0.25), 1.0);
...
uniform float u_p_mx;
uniform float u_p_my;
...
void fs_main(input: VertexOutput){
    ...
    // stage 1: circle(...)
    float sdf_result = sdf_circle(p, 0.15);
    ...
    // stage 4: vignette(...)
    color_result = vec4(color_result.rgb * (1.0 - length(uv) * 0.25), 1.0);
...
class MouseFollow extends HTMLElement {
  ...
  _frame() {
    ...
    // Update param values
    this._paramValues[0] = (this._arcBaseOverrides?.[0] ?? 0) + ((mouseX * 2.0) - 1.0);
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 0) + ((mouseY * 2.0) - 1.0);
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_mx');
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_my');
