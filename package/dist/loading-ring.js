// loading-ring.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component with WebGL2 fallback.
//   <script type="module" src="./loading-ring.js"></script>
...
void fs_main(input: VertexOutput){
    ...
    float angle = u_p_angle;

    vec2 p = vec2(uv.x * aspect, uv.y);
    // stage 0: ring(...)
    float sdf_result = abs(length(p) - 0.3) - 0.04;

    // stage 1: mask_arc(...)
    float arc_theta = atan2(p.x, p.y) + 3.14159265359;
    sdf_result = (arc_theta < angle ? sdf_result : 999.0);
...
class LoadingRing extends HTMLElement {
  ...
  _frame() {
    ...
    this._paramValues[0] = (this._arcBaseOverrides?.[0] ?? 0) + (data_progress * 6.283);
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_angle');
