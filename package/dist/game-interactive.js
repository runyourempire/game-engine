// game-interactive.js — Generated by GAME compiler v0.2.0
// Zero-dependency WebGPU component with WebGL2 fallback.
//   <script type="module" src="./game-interactive.js"></script>
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    // ── Layer 0: fluid ──
    {
        ...
        // stage 0: curl_noise(...)
        let curl_offset = curl2(p, freq, amp);
        ...
        // stage 2: tint(...)
        var color_result = vec4f(vec3f(glow_result) * vec3f(0.5, 0.0, 1.0), 1.0);
    ...
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: concentric_waves(...)
        var sdf_result = sin(length(p) * 10.0 - time * 3.0) * exp(-length(p) * 2.0);
        ...
        let lc = color_result.rgb;
        final_color = vec4f(final_color.rgb + lc * 0.600, 1.0);
    }
...
void fs_main(input: VertexOutput){
    ...
    // ── Layer 0: fluid ──
    {
        ...
        // stage 0: curl_noise(...)
        float curl_offset = curl2(p, freq, amp);
        float sdf_result = length(curl_offset) - 0.01;

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, 3.0);

        // stage 2: tint(...)
        vec4 color_result = vec4(vec3(glow_result) * vec3(0.5, 0.0, 1.0), 1.0);
    ...
    // ── Layer 1: ripples ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: concentric_waves(...)
        float sdf_result = sin(length(p) * 10.0 - time * 3.0) * exp(-length(p) * 2.0);
        ...
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc * 0.600, 1.0);
    }
...
class GameInteractive extends HTMLElement {
  ...
  _frame() {
    ...
    // Update param values
    this._paramValues[0] = (this._arcBaseOverrides?.[0] ?? 3) + (mouseX * 4.0);
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 0.5) + mouseY;
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_freq');
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_amp');
...
customElements.define('game-interactive', GameInteractive);
export { GameInteractive };
