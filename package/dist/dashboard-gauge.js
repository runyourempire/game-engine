...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    let center_glow = u.p_center_glow;
    ...
    // ── Layer 0: background ──
    {
        ...
        // stage 0: ring(...)
        var sdf_result = abs(length(p) - 0.35) - 0.025;

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, 0.8);
    ...
    // ── Layer 1: fill_arc ──
    {
        ...
        sdf_result = select(999.0, sdf_result, arc_theta < fill);
    ...
    // ── Layer 2: center_dot ──
    {
        var p = vec2f(uv.x * aspect, uv.y);
        // stage 0: circle(...)
        var sdf_result = sdf_circle(p, 0.04);

        // stage 1: glow(...)
        let glow_result = apply_glow(sdf_result, center_glow);

        // stage 2: tint(...)
        var color_result = vec4f(vec3f(glow_result) * vec3f(1.0), 1.0);
...
uniform float u_p_fill;
uniform float u_p_intensity;
uniform float u_p_center_glow;
...
void fs_main(input: VertexOutput){
    ...
    float center_glow = u_p_center_glow;
    ...
    // ── Layer 0: background ──
    {
        ...
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - 0.35) - 0.025;

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, 0.8);
    ...
    // ── Layer 1: fill_arc ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: ring(...)
        float sdf_result = abs(length(p) - 0.35) - 0.03;

        // stage 1: mask_arc(...)
        float arc_theta = atan2(p.x, p.y) + 3.14159265359;
        sdf_result = (arc_theta < fill ? sdf_result : 999.0);
    ...
    // ── Layer 2: center_dot ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        // stage 0: circle(...)
        float sdf_result = sdf_circle(p, 0.04);

        // stage 1: glow(...)
        float glow_result = apply_glow(sdf_result, center_glow);

        // stage 2: tint(...)
        vec4 color_result = vec4(vec3(glow_result) * vec3(1.0), 1.0);
...
class DashboardGauge extends HTMLElement {
  ...
  _frame() {
    ...
    this._paramValues[1] = (this._arcBaseOverrides?.[1] ?? 2) + (data_value * 4.0);
    this._paramValues[2] = (this._arcBaseOverrides?.[2] ?? 1.5) + (data_value * 3.0);
  ...
  _initWebGL2() {
    ...
    this._glParamUniforms[0] = gl.getUniformLocation(prog, 'u_p_fill');
    this._glParamUniforms[1] = gl.getUniformLocation(prog, 'u_p_intensity');
    this._glParamUniforms[2] = gl.getUniformLocation(prog, 'u_p_center_glow');
